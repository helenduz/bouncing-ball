...
const MAX_SPEED = 5; // pixel per REFRESH_INTERVAL
const NUM_BALLS = 20;
const RADIUS = 30;
...
const shiftBallColor = (i) => {
    ...
    var newColor = colorsArray[getRandomInt(0, colorsArray.length - 1)];
    while (newColor === curColor) {
        newColor = getRandomInt(0, colorsArray.length - 1);
    }
...
const start = () => {
    initialize();
    d3.interval(transition, REFRESH_INTERVAL);
};
